...
//   x: number;
//   y: number;
//   constructor(id: number, S_id: number, id: number, itemId: number, category: string, imgUrl: string, x: number, y: number) {
//     this.id = id;
//     this.S_id = S_id;
//     this.id = id;
//     this.itemId = itemId;
...
//   Situation: string;
//   id: number;
//   constructor(S_id: number, Situation: string, id: number) {
//     this.S_id = S_id;
//     this.id = id;
...
//   name: string;
//   imgUrl: string;
//   constructor(S_id: number, id: number, name: string, imgUrl: string) {
//     this.S_id = S_id;
//     this.id = id;
//     this.name = name;
//     this.imgUrl = imgUrl;
...
//     this.db.executeSql(`CREATE TABLE IF NOT EXISTS items (
...
//       this.db.executeSql(`CREATE TABLE IF NOT EXISTS Patients (
//                 id  INTEGER PRIMARY KEY AUTOINCREMENT,
//                 name  TEXT,
//                 photo TEXT
//               )`, {})
//       .then( () => console.log('Table Patients created'))

//         this.db.executeSql(`CREATE TABLE IF NOT EXISTS Situations(
//                 S_id        INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
//                 Situation   TEXT NOT NULL,
//                 id        INTEGER,
//                 FOREIGN KEY (id) REFERENCES Patients(id)
//               )`, {})
//           .then( () => console.log('Table Situations created'))

//             this.db.executeSql(`CREATE TABLE IF NOT EXISTS itemsPosition (
//                 id        INTEGER PRIMARY KEY AUTOINCREMENT,
//                 S_id      INTEGER,
//                 itemId    INTEGER,
//                 category  TEXT,
...
//                 y         INTEGER
//               )`, {})
//           .then( () => console.log('Table itemsPosition created'))
...
//     return this.db.executeSql(sql, [patient.id]);
//   }

//   public editPatient(patient: Patient) {
//     let sql = `UPDATE Patients SET name = ? WHERE id = ? `;
//     return this.db.executeSql(sql, [patient.name, patient.id]);
//   }

//   public lastPatientAdded() {
//     let sql = `SELECT id FROM Patients
...
//     let sql = `INSERT INTO Situations (id, Situation) VALUES (?,?);`;
//     return this.db.executeSql(sql, [patient.id, environment.name]);
//   }

//   public getSituations(currentPatient: Patient) {
//     let sql = `SELECT s.S_id, s.id, i.name, i.imgUrl
//                 FROM Situations AS s
//                 JOIN itemsPosition AS ip ON s.S_id = ip.S_id
//                 JOIN items AS i ON ip.itemId = i.id
//                 WHERE s.id = ? AND i.category = ?`;

//     return this.db.executeSql(sql, [currentPatient.id, "background"]).then(response => {
//       let situations = [];
//       for (let i = 0; i < response.rows.length; i++) {
...
//     let sql = `SELECT i.imgUrl AS imgUrl, i.category AS category, s.id AS id, p.id AS id, p.S_id AS S_id, p.itemId AS itemId, p.x AS x, p.y AS y
//                 FROM Situations AS s
